...
pub struct CheckAuthApi;


impl<S, B> Transform<S, ServiceRequest> for CheckAuthApi
    where
        ...
        B: 'static,
{
    ...
    type Error = Error;
    type InitError = ();
    type Transform = CheckAuthApiMiddleware<S>;
    type Future = Ready<Result<Self::Transform, Self::InitError>>;

    fn new_transform(&self, service: S) -> Self::Future {
        ready(Ok(CheckAuthApiMiddleware { service: Rc::new(service) }))
    }
}

pub struct CheckAuthApiMiddleware<S> {
    service: Rc<S>,
}
...
impl<S, B> Service<ServiceRequest> for CheckAuthApiMiddleware<S>
    where
        S: Service<ServiceRequest, Response = ServiceResponse<B>, Error = Error>+ 'static,
        S::Future: 'static,
        B: 'static,
